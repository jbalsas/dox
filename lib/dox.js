...
var _resolveClassPath = function(clazz, rest, url) {
  var path = clazz.replace(".", "/");
...
exports.processLinkTag = function(options, match, symbol, label) {
  var url               = symbol,
      resolveClassPath  = options.resolveClassPath || _resolveClassPath;

  label = label || symbol.replace(/[:#]/g, ' ').trim().replace(/\s+/g, '.');

  if (url.indexOf("http") !== 0 && url.indexOf("https://") !== 0 && url.indexOf("www.") !== 0) {
      var parts = url.split("#"),
          clazz = parts.shift(),
          rest  = parts.length ? parts.join() : "";

      url = resolveClassPath(clazz, rest, url);
  }

  return '<a href="' + url +'">' + label + '</a>';
}

exports.processInlineTags = function(str, options) {
  var LINK_REGEX_SIMPLE = /{@link ([^|}]*)}/g,
      LINK_REGEX_LABEL  = /{@link ([^|}]*)\|([^|}]*)}/g;

  str = str.replace(LINK_REGEX_SIMPLE, function(match, symbol) {
    return exports.processLinkTag(options, match, symbol)
  });

  str = str.replace(LINK_REGEX_LABEL, function(match, symbol, label) {
    return exports.processLinkTag(options, match, symbol, label)
  });
...
exports.parseComment = function(str, options) {
  ...
  str = exports.processInlineTags(str, options);
  ...
  if (~str.indexOf('\n@')) {
    ...
    comment.tags = tags.map(function(str) { return exports.parseTag(str, options); });
...
exports.parseTag = function(str, options) {
  var tag = {}
    , types = []
    , parts = str.split(/[\s\n]+/)
    , type = tag.type = parts.shift().replace('@', '');

  if (type == 'param' || type == 'return' || type == 'type' || type === 'typedef') {
  ...
  switch (type) {
    ...
    case 'return':
      ...
      break;
    case 'see':
      var namePath = /@see ([^\s]*)$/.exec(str);

      if (namePath) {
        tag.string = exports.processLinkTag(options, namePath, namePath[1]);
      } else {
        tag.string = /@see (.*)/.exec(str)[1];
      }
